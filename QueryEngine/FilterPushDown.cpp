...
class FilterPushDown {
    ...
    if(rex_operator) {
      ...
      if(oper_size == 2) {
        ...
        if(rex_input_lhs && rex_input_rhs) {
          unsigned t_id_lhs, t_id_rhs; std::string col_name_lhs, col_name_rhs;
          ...
          for(auto node : nodes) {
            ...
            if(node_scan) {
              ...
              if(t_id == t_id_lhs) {
                ...
                auto col_desc = 
                  ra_executor->getSessionInfo().get_catalog().getMetadataForColumn(node_scan->getTableDescriptor()->tableId, col_name_lhs);
                if(!col_desc->columnType.is_integer() &&
                   !col_desc->columnType.is_time() &&
  ...
  bool evaluateAndPushDown(std::vector<std::shared_ptr<RelAlgNode>>& nodes,
    ...
    for(size_t i = 0; i < nodes.size(); i++) {
      ...
      if(node_scan) {
        ...
        if(!std::dynamic_pointer_cast<RelScan>(nodes[i+1]) &&
           !std::dynamic_pointer_cast<RelJoin>(nodes[i+1])) {
          continue;
        }      
        ...
        auto node_filter_new = 
          std::shared_ptr<RelAlgNode>(new RelFilter(filter_expr_new, nodes[i]));
        ...
        if(executeFilterAndEvaluate(nodes[i+1].get(), num_tuples, ra_executor)) {
 ...
 private:
  std::pair<unsigned, std::string> findColumnNameByIndex(const RelAlgNode* source, unsigned c_id) {
    std::vector<std::pair<unsigned, unsigned>> t_num_cols; std::vector<std::string> schema;
  ...
  std::unique_ptr<const RexScalar> findFilterAndProject(
    ...
    for(auto it_nodes = nodes.rbegin(); it_nodes != nodes.rend(); ++it_nodes) {
      ...
      if(node_filter) {
        filter_expr_new = 
          findFilterAndProjectFromRex(node_filter->getCondition(), nodes[scan_idx], cols_to_project);
        continue;
      }
  ...
  std::unique_ptr<const RexScalar> findFilterAndProjectFromRex(const RexScalar* rex,
    ...
    if(rex_operator) {
      ...
      for(size_t i = 0; i < oper_size; i++) {
        auto operand_new = 
          findFilterAndProjectFromRex(rex_operator->getOperand(i), node_scan, cols_to_project);
        if(operand_new) {
          operands_new.push_back(std::move(operand_new));
        }
      }
    ...
    const auto rex_case = dynamic_cast<const RexCase*>(rex);
    if(rex_case) {
      std::vector<std::pair<std::unique_ptr<const RexScalar>,
                            std::unique_ptr<const RexScalar>>> expr_pair_list_new;
      for(size_t i = 0; i < rex_case->branchCount(); i++) {
        auto oper_when_new = 
          findFilterAndProjectFromRex(rex_case->getWhen(i), node_scan, cols_to_project);
        auto oper_then_new = 
          findFilterAndProjectFromRex(rex_case->getThen(i), node_scan, cols_to_project);
      ...
      if(!expr_pair_list_new.empty()) {
        auto else_expr_new =
          findFilterAndProjectFromRex(rex_case->getElse(), node_scan, cols_to_project);
        if(else_expr_new) {
  ...
  void makeAndInsertCompound(std::vector<std::shared_ptr<RelAlgNode>>& nodes, size_t filter_idx, 
    size_t output_idx, std::vector<std::pair<unsigned, std::string>>& cols_to_project) {
    ...
    for(auto col : cols_to_project) {
      auto rex_col = std::unique_ptr<const RexScalar>(
        new RexInput(nodes[filter_idx]->getInput(0), col.first));
    ...
    // create compound
    auto node_compound =
      std::make_shared<RelCompound>(filter_rex, target_exprs, groupby_count, agg_exprs, fields, scalar_sources, false);
  ...
  bool executeFilterAndEvaluate(const RelAlgNode* node, size_t num_tuples,
    RelAlgExecutor* ra_executor) {
    auto executor = Executor::getExecutor(
      ra_executor->getSessionInfo().get_catalog().get_currentDB().dbId);
    ...
    try {
      auto execution_result = ra_executor_sub.executeRelAlgQueryFPD(node, fpd_max_count);
    ...
    for(auto it_nodes = nodes.rbegin(); it_nodes != nodes.rend(); ++it_nodes) {
      ...
      if(node_project) {
        ...
        for(size_t i = 0; i < node_project->size(); i++) {
          ...
          auto rex_new = 
            buildNewProjectExpr(rex, nodes[scan_idx], nodes[output_idx], input_idx, cols_to_project);
      ...
      if(node_filter) {
        ...
        auto rex_new =
          buildNewFilterExpr(rex, nodes[scan_idx], nodes[output_idx], input_idx, cols_to_project);
  ...
  std::unique_ptr<const RexScalar> buildNewProjectExpr(const RexScalar* rex,
    ...
    if(rex_input) {
      ...
      std::vector<std::pair<unsigned, unsigned>> t_num_cols; std::vector<std::string> schema;
      std::tie(t_num_cols, schema) = getSchemaFromSource(source);
      int num_cols_so_far = 0; int num_modifier = 0;
      for(auto it = t_num_cols.begin(); it != t_num_cols.end(); ++it) {
        if(c_id < num_cols_so_far + it->second) {
          if(it->first == node_scan->getId()) {
            auto it_find = std::find_if(cols_to_project.begin(), cols_to_project.end(),
              [&c_id, &num_cols_so_far](auto& col) {
            ...
            if(node_output->getInput(input_idx)->getInput(0) == source) {
              return std::unique_ptr<const RexScalar>(
                new RexInput(node_output->getInput(input_idx), c_id_new));
            } else {
    ...
    if(rex_operator) {
      std::vector<std::unique_ptr<const RexScalar>> operands_new;
      for(size_t i = 0; i < rex_operator->size(); i++) {        
        auto oper_new = buildNewProjectExpr(rex_operator->getOperandAndRelease(i),
    ...
    if(rex_case) {
      std::vector<std::pair<std::unique_ptr<const RexScalar>,
                            std::unique_ptr<const RexScalar>>> expr_pair_list_new;
      for(size_t i = 0; i < rex_case->branchCount(); i++) {
        auto oper_when_new = buildNewProjectExpr(rex_case->getWhen(i),
                                                 node_scan, node_output, input_idx, cols_to_project);
        auto oper_then_new = buildNewProjectExpr(rex_case->getThen(i),
                                                 node_scan, node_output, input_idx, cols_to_project);
        auto expr_pair_new = std::make_pair(std::move(oper_when_new), std::move(oper_then_new));
        expr_pair_list_new.push_back(std::move(expr_pair_new));
      }
      auto else_expr_new = buildNewProjectExpr(rex_case->getElse(),
                                               node_scan, node_output, input_idx, cols_to_project);
      return std::unique_ptr<const RexScalar>(new RexCase(expr_pair_list_new, else_expr_new));
    }
    const auto rex_ref = dynamic_cast<const RexRef*>(rex);
  ...
  std::unique_ptr<const RexScalar> buildNewFilterExpr(const RexScalar* rex,
    ...
    if(rex_operator) {
      ...
      for(size_t i = 0; i < oper_size; i++) {
        auto operand_new = 
          buildNewFilterExpr(rex_operator->getOperand(i),
                             node_scan, node_output, input_idx, cols_to_project);
        if(operand_new) {
          operands_new.push_back(std::move(operand_new));
        }
    ...
    if(rex_case) {
      std::vector<std::pair<std::unique_ptr<const RexScalar>,
                            std::unique_ptr<const RexScalar>>> expr_pair_list_new;
      for(size_t i = 0; i < rex_case->branchCount(); i++) {
        auto oper_when_new = buildNewFilterExpr(rex_case->getWhen(i),
                                                 node_scan, node_output, input_idx, cols_to_project);
        auto oper_then_new = buildNewFilterExpr(rex_case->getThen(i),
                                                 node_scan, node_output, input_idx, cols_to_project);
        auto expr_pair_new = std::make_pair(std::move(oper_when_new), std::move(oper_then_new));
        expr_pair_list_new.push_back(std::move(expr_pair_new));
      }
      auto else_expr_new = buildNewFilterExpr(rex_case->getElse(),
                                               node_scan, node_output, input_idx, cols_to_project);
      return std::unique_ptr<const RexScalar>(new RexCase(expr_pair_list_new, else_expr_new));
    }
    const auto rex_ref = dynamic_cast<const RexRef*>(rex);
  ...
  std::unordered_map<const RelAlgNode*, std::pair<std::vector<std::pair<unsigned, unsigned>>, std::vector<std::string>>> schema_map_;
};
...
bool push_down_filter_predicates(std::vector<std::shared_ptr<RelAlgNode>>& nodes,
  ...
  for(auto it_nodes = nodes.begin(); it_nodes != nodes.end(); ++it_nodes) {
    ...
    if(node_scan) {
      ...
      auto num_tuples = fpd.getNumTuples(node_scan->getTableDescriptor()->tableId,
